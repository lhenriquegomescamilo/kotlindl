...
internal class LogCoshTest {
    ...
    fun simpleTest2() {
        ...
        EagerSession.create().use { session ->
            val tf = Ops.create(session)
            val instance = LogCosh(reductionType = ReductionType.SUM_OVER_BATCH_SIZE)
    ...
    fun sumReduction() {
        ...
        EagerSession.create().use { session ->
            val tf = Ops.create(session)
            val instance = LogCosh(reductionType = ReductionType.SUM)
